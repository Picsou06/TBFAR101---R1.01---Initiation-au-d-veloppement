...
void initTableauRandom(int tableau[], int taille) {
    ...
    for (int i = 0; i < taille; i++) {
        tableau[i] = rand() % 5; // Valeurs aléatoires entre 0 et 50
    }
}
...
int main() {
    ...
    while(findValue(tableau, taille, value) != -1)
    {
        taille = suppElemPos(tableau, taille, findValue(tableau, taille, value));
    }

    printf("Taille final: %i", taille);
    printf("\n");
